...
namespace GAM.Controllers
{
    ...
    public class EmpresasController : BaseController
    {
        ...
        public ActionResult Editar(string Id)
        {
            ...
            return View(n.empresas.Seleccionar_PorId(Id));
        }
        ...
        /// <returns></returns>
        [HttpPost]
        public ActionResult Modificar(string Nombre, string Sucursal, string RFC, string Direccion, string Ciudad,  string Estado, int IdEmpresa)
        {
            UsuariosRoles usuariorolsesion = (UsuariosRoles)Session["GranSesion"];

            Empresas items = new Empresas();
            items.Nombre = Nombre;
            items.Sucursal = Sucursal;
        ...
        {
            return View(n.documentosempresas.Seleccionar_PorId(Id));
        }

        [HttpPost]
        public ActionResult GuardarNotas(string Id, string Notas, string PalabrasClave)
        {
            n.documentosempresas.Modificar_Notas(Notas, PalabrasClave, Id);
            var idempresa = n.documentosempresas.Seleccionar_PorId(int.Parse(Id)).IdEmpresa;
            return Redirect("/Empresas/Editar?Id=" + idempresa);
        }
        ...
        public JsonResult Guardar(string Nombre, string Sucursal, string RFC, string TipoEmpresa, string Direccion, string Ciudad, string Estado, string IdConfiguracion)
        {
            Empresas items = new Empresas();
            items.Nombre = Nombre;
            items.Sucursal = Sucursal;
        ...
        string Prospecto, string Cliente, string Competidor, string Sector, string Id, string RFC, string UDN)
        {
            ...
            int salida = 0;
            if (n.empresas.Modificar_Registro(items) == -1)
                salida = 1;
            return Json(salida, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult EmpresasDetalleMostrar(int idempresa)
        {
            return Json(n.empresas.Seleccionar_PorId(idempresa.ToString()), JsonRequestBehavior.AllowGet);
        }

        public JsonResult EmpresaEnOportunidades(string idempresa)
        {
            return Json(n.oportunidades.Seleccionar_EmpresasEnOportunidades(idempresa), JsonRequestBehavior.AllowGet);
        }

        public JsonResult CargaInicialArchivos(int idempresa)
        {
            return Json(n.documentosempresas.Seleccionar_Registros(idempresa), JsonRequestBehavior.AllowGet);
        }

        public JsonResult DocumentoSeleccionarPorId(int Id)
        {
            return Json(n.documentosempresas.Seleccionar_PorId(Id), JsonRequestBehavior.AllowGet);
        }
       
        public JsonResult QuitarArchivo(int iddocumento, int idempresa)
        {
            var _NombreArchivo = n.documentosempresas.Seleccionar_PorNombre(iddocumento);
           
                //System.IO.File.Delete(_ruta);
                //Eliminar tambien de la bd
                n.documentosempresas.Eliminar_Registro(iddocumento);
        ...
        public JsonResult SubirArchivo()
        {
            ...
            if (Request.Files.Count > 0)
            {
                try
                {
                    ...
                    var idemp = Request.Form["idempresa"];
                    ...
                    int consecutivo = n.documentosempresas.Seleccionar_Consecutivo(int.Parse(idemp)) == 0 ? 1 : n.documentosempresas.Seleccionar_Consecutivo(int.Parse(idemp));
                    fileName = nombrearchivo + "_E" + idemp + "_C" + consecutivo.ToString() + extension;
                    ...
                    if (System.IO.File.Exists(ruta1))
                    {
                        ...
                        consecutivo = n.documentosempresas.Seleccionar_ConsecutivoIncrementa(int.Parse(idemp));
                        fileName = nombrearchivo + "_E" + idemp + "_C" + consecutivo + extension;
                        ruta1 = Path.Combine(Server.MapPath("~/Documentos/"), fileName);
                    }

                    //Guardar en la tabla correspondiente (Archivos) /////////////////////////////////////////////////////////////////////////verificar aqui
                    Models.Models.DocumentosEmpresas items = new Models.Models.DocumentosEmpresas();
                    //DocumentosEmpresas items = new DocumentosEmpresas();
                    items.Nombre = fileName;
                    items.IdEmpresa = int.Parse(idemp);
                    ...
                    n.documentosempresas.Agregar_Registro(items);

                    //Guardar el archivo
                    file.SaveAs(ruta1);

                    //Devolver los archivos
                    return Json(n.documentosempresas.Seleccionar_Registros(int.Parse(idemp)), JsonRequestBehavior.AllowGet);
                }
        ...
        public JsonResult Quitarempresa(string idempresa)
        {
            var datos = n.empresas.Eliminar_Registro(idempresa);
