...
namespace GAM.Models.Repository
{
    ...
    public class UsuariosRepositorio
    {
        ...
        {
            b.ExecuteCommandQuery("update usuarios  set Activo = 0 where Id = @id");
        ...
        {
            b.ExecuteCommandQuery("SELECT usu.id, usu.Nombre, usu.ApellidoPaterno, ISNULL(usu.ApellidoMaterno, '') AS ApellidoMaterno, usu.Clave, usu.Activo, usu.Correo, conf.Nombre AS Empresa " +
            "FROM usuarios usu " +
        ...
        {
            string consulta = "SELECT usu.id, LTRIM(usu.nombre) AS Nombre, usu.ApellidoPaterno, usu.ApellidoMaterno, usu.correo, usu.clave, usu.Telefono  " +
            "FROM usuarios usu " +
            "INNER JOIN UsuarioConfiguracion usuc ON usuc.IdUsuario=usu.id " +
            "INNER JOIN UsuariosRoles ur ON ur.IdUsuario=usu.Id " +
            "LEFT JOIN UsuariosDetalle ud ON ud.IdUsuario=usu.Id " +
        ...
        protected List<Usuarios> SeleccionarUsuariosResponsables(string idconfiguracion, string idempresa, string idusuario, string idrol)
        {
            ...
            {
                consulta_ant = "SELECT usu.id, LTRIM(usu.nombre) AS nombre, usu.ApellidoPaterno, usu.ApellidoMaterno " +
                "FROM usuarios usu " +
                "INNER JOIN UsuarioConfiguracion usuc ON usuc.IdUsuario = usu.id " +
                "INNER JOIN UsuariosRoles ur ON ur.IdUsuario = usu.Id " +
                "WHERE usuc.IdConfiguracion = 3 " +
                ...
                "AND usu.id <> 3 " +
                "AND ur.idrol = 5 " +
                "ORDER BY ur.idrol, usu.nombre";

                consulta = "SELECT usu.id, LTRIM(usu.nombre) AS nombre, usu.ApellidoPaterno, usu.ApellidoMaterno, " +
                "CASE " +
                "WHEN rl.id = 3 THEN 'Responsable' " +
                "WHEN rl.id = 5 THEN 'Proveedor' " +
                "END AS Rol " +
                "FROM usuarios usu " +
                "INNER JOIN UsuarioConfiguracion usuc ON usuc.IdUsuario = usu.id " +
                "INNER JOIN UsuariosRoles ur ON ur.IdUsuario = usu.Id " +
            ...
            while (reader.Read())
            {
                ...
                item.Id              = int.Parse(reader["Id"].ToString());
                item.Nombre          = reader["nombre"].ToString();
                item.ApellidoPaterno = reader["rol"].ToString();
                item.ApellidoMaterno = reader["ApellidoMaterno"].ToString();
                resultado.Add(item);
            }
            reader = null;
        ...
        protected List<Usuarios> SeleccionarUsuariosPreGuardado(string idconfiguracion, string Nombre, string Correo, string RFC)
        {
            string consulta = "" +
            "SELECT LTRIM(usu.nombre) AS Nombre, usu.correo, usd.rfc " +
            "FROM usuarios usu " +
            "INNER JOIN usuarioconfiguracion uc ON uc.idusuario=usu.id " +
            "INNER JOIN usuariosdetalle usd ON usd.idusuario=usu.id " +
            "WHERE usu.nombre LIKE @nombre OR usu.correo LIKE @correo OR usd.rfc LIKE @rfc " +
            ...
            while (reader.Read())
            {
                ...
                item.Nombre = reader["nombre"].ToString();
                item.Correo = reader["correo"].ToString();
                item.Clave = reader["rfc"].ToString();
                resultado.Add(item);
            }
            reader = null;
            b.ConnectionCloseToTransaction();
        ...
        {
            b.ExecuteCommandQuery("SELECT usu.id,usu.nombre, usu.apellidopaterno, ISNULL(usu.apellidomaterno, '') AS ApellidoMaterno, usu.Correo, usu.clave, usu.contraseña, usu.Activo, " +
            "rol.id as IdRol, rol.nombre AS NombreRol, " +
        ...
        protected int Agregar(string Nombre, string ApellidoPaterno, string ApellidoMaterno, string Correo, string Clave, string Contraseña)
        {
            ...
            "BEGIN " +
            "INSERT INTO usuarios (nombre,apellidopaterno,apellidomaterno,correo,clave,contraseña,activo) " +
            "VALUES(@nombre, @apaterno, @amaterno, @correo, @clave, @contraseña, 1); " +
            "SELECT @@IDENTITY " +
            "END " +
            "ELSE " +
            "SELECT 0");
        ...
        protected int AgregarProveedor(string Nombre, string Correo, string Telefono, string Celular, string Empresa, string Direccion, string Ciudad, string Notas, string Rol, string IdConfiguracion, string FisicaMoral, string RFC, string InternoExterno, string CreadoPor)
        {
            string consulta = "DECLARE @idcreado INT; " +
            "BEGIN TRANSACTION " +
            "   BEGIN TRY " +
            "      BEGIN " +
            "          INSERT INTO usuarios (nombre,correo,clave,contraseña,activo) " +
            "          VALUES(@nombre, @correo, @clave, @contraseña, 1); " +
            "          SET @idcreado = @@IDENTITY; " +
            "          INSERT INTO usuariosdetalle(IdUsuario,telefono,celular,empresa,direccion,ciudad,notas,fisicamoral,rfc,internoexterno,creadopor) " +
            "          VALUES(@idcreado,@telefono,@celular,@idempresa,@direccion,@ciudad,@notas,@fisicamoral,@rfc,@internoexterno,@creadopor); " +
            "          INSERT INTO usuarioconfiguracion VALUES(@idcreado,3); " +
            "          INSERT INTO usuariosroles (idusuario, idrol) VALUES(@idcreado, 5); " +
            "          INSERT INTO empresasusuarios (idempresa,idusuario,activo) VALUES(@idempresa,@idcreado,1); " +
            "          COMMIT TRANSACTION; " +
            "      END " +
            "   END TRY " +
            "BEGIN CATCH " +
            ...
            if (string.IsNullOrEmpty(Empresa))
                b.AddParameter("@idempresa");
            else
                b.AddParameter("@idempresa", Empresa, SqlDbType.Int);
            ...
            b.AddParameter("@rfc", RFC.ToUpper(), SqlDbType.NVarChar, 18);
            b.AddParameter("@internoexterno", InternoExterno, SqlDbType.Int);
            b.AddParameter("@creadopor", CreadoPor, SqlDbType.Int);
            return b.InsertUpdateDelete();
        }

        protected int AgregarDetalle(string IdUsuario, string Telefono, string Celular, string Empresa, string Direccion, string Ciudad, string Notas, string FisicaMoral, string RFC, string InternoExterno, string CreadoPor)
        {
            ...
            if (!string.IsNullOrEmpty(InternoExterno))
                b.AddParameter("@internoexterno", InternoExterno, SqlDbType.Int);
            b.AddParameter("@creadopo", CreadoPor, SqlDbType.Int);
            return b.InsertUpdateDelete();
        }

        protected int ModificarDetalle(string Telefono, string Celular, string Direccion, string Ciudad, string Notas, string IdUsuario, string FisicaMoral, string RFC, string InternoExterno)
        {
            string consulta = "UPDATE usuariosDetalle SET telefono=@telefono, celular=@celular, direccion=@direccion, ciudad=@ciudad, notas=@notas, fisicamoral=@fisicamoral, rfc=@rfc, internoexterno=@internoexterno WHERE idusuario=@idusuario";
            b.ExecuteCommandQuery(consulta);
            b.AddParameter("@telefono", Telefono, SqlDbType.NVarChar);
            b.AddParameter("@celular", Celular, SqlDbType.NVarChar);            
            b.AddParameter("@direccion", Direccion, SqlDbType.NVarChar);
        ...
        protected int Modificar(string eNombre, string eApellidoPaterno, string eApellidoMaterno, string eCorreo, string eClave, string eContraseña, string eActivo, string eRol, string eId)
        {
            int regreso = 0;
            //Actualizar el detalle del usuario
            b.ExecuteCommandQuery("UPDATE usuarios SET nombre=@nombre, apellidopaterno=@apellidopaterno, apellidomaterno=@apellidomaterno, correo=@correo, clave=@clave, contraseña=@contraseña, activo=@activo WHERE id=@id");
        ...
        protected int ModificarGerente(string eNombre, string eClave, string eTelefono, string eId)
        {
            //Actualizar el detalle del Gerente
            b.ExecuteCommandQuery("UPDATE usuarios SET nombre=@nombre, correo=@clave, telefono=@telefono WHERE id=@id");
            b.AddParameter("@nombre", eNombre.ToUpper(), SqlDbType.NVarChar);
            b.AddParameter("@clave", eClave, SqlDbType.NVarChar);
            b.AddParameter("@telefono", eTelefono, SqlDbType.NChar, 10);
            b.AddParameter("@id", eId, SqlDbType.Int);
            return b.InsertUpdateDelete();
        }
