...
namespace GAM.Models.Repository
{
    ...
    public class OportunidadesResponsablesRepositorio
    {
        ...
        protected List<UsuariosEmpresasOportunidadesDetalle> SeleccionarResponsablesPorOportunidadPorRol(string id, string idconfiguracion, string idrol)
        {
            string consulta = "SELECT opor.id, " +
            ...
            "INNER JOIN UsuariosRoles ur ON ur.IdUsuario=usu.id " +
            "WHERE opor.id = @id " +
            "AND ur.idrol IN (3, 5)"; //Si todos los involucrados deberían comunicarse, agregar que los roles son 3 y 5 IN (3, 5)
            b.ExecuteCommandQuery(consulta);
            b.AddParameter("@id", id, SqlDbType.Int);
